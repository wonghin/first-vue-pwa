...
<template>
    <Container ref="scrollContainer" @scroll="scrolling">
        ...
        <v-row v-if="data" class="mt-8" v-for="(page, index) in data?.pages" :key="index">
            <v-col class="d-flex justify-center" :cols="12" sm="6" md="4" v-for="item in page.pageData.results"
                :key="item.name">
